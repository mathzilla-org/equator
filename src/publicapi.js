/*********************************************************
 * The publicly exposed Equator API.
 ********************************************************/
...
function insistOnInterVer() {
  if (window.console) console.warn(
    'You are using the Equator API without specifying an interface version, ' +
    'which will fail in v1.0.0. Easiest fix is to do the following before ' +
    'doing anything else:\n' +
    '\n' +
    '    Equator = Equator.getInterface(1);\n' +
    '    // now Equator.MathField() works like it used to\n' +
    '\n' +
...
Equator.prototype = Progenote.p;
Equator.VERSION = "{VERSION}";
Equator.interfaceVersion = function(v) {
  // shim for #459-era interface versioning (ended with #495)
  if (v !== 1) throw 'Only interface version 1 supported. You specified: ' + v;
  insistOnInterVer = function() {
    if (window.console) console.warn(
      'You called Equator.interfaceVersion(1); to specify the interface ' +
...
Equator.getInterface = getInterface;
...
function getInterface(v) {
  ...
  var AbstractEquator = APIClasses.AbstractEquator = P(Progenote, function(_) {
    ...
    _.__mathquillify = function(classNames) {
      ...
      this.revert = function() {
        return el.empty().unbind('.equator')
    ...
    _.html = function() {
      return this.__controller.root.jQ.html()
        .replace(/ equator-(?:command|block)-id="?\d+"?/g, '')
  ...
  MQ.prototype = AbstractEquator.prototype;

  APIClasses.EditableField = P(AbstractEquator, function(_, super_) {
  ...
  /**
   * Export the API functions that Equator-ify an HTML element into API objects
   * of each class. If the element had already been Equator-ified but into a
...
Equator.noConflict = function() {
  window.Equator = origEquator;
  return Equator;
};
var origEquator = window.Equator;
window.Equator = Equator;
