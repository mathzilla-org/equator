...
The animated demo on <equator.com> has this behavior.
...
Nested content in latex rendered during initialization or pasted into equator is truncated to avoid violating `maxDepth`. When `maxDepth` is not set, no depth limit is applied by default.
...
The `*OutOf` handlers are called when a cursor movement would cause the cursor to leave the Equator mathField. These let you integrate cursor movement seamlessly between your code and Equator. For example, when the cursor is at the right edge, pressing the Right key causes the `moveOutOf` handler to be called with `MQ.R` and the math field API object. Pressing Backspace causes `deleteOutOf` to be called with `MQ.L` and the API object.
...
To change the foreground color, set both `color` and the `border-color` because some Equator symbols are implemented with borders instead of pure text.
...
To support a Equator editable background color other than white in IE8, set the background color on both the editable mathField and on elements with class `mq-matrixed`. Then set a Chroma filter on elements with class `mq-matrixed-container`.
