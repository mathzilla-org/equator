suite('Public API', function() {
  suite('global functions', function() {
    ...
    test('MQ.MathField()', function() {
      ...
      assert.ok(mathField instanceof Equator);
    });
    ...
    test('interface versioning isolates prototype chain', function() {
      ...
      var MQ1 = Equator.getInterface(1);
  ...
  suite('*OutOf handlers', function() {
    ...
    testHandlers('MQ.StaticMath() constructor', function(options) {
      return MQ.StaticMath($('<span>\\EquatorMathField{}</span>').appendTo('#mock')[0], options).innerFields[0];
    });
    testHandlers('MQ.MathField::config()', function(options) {
      return MQ.MathField($('<span></span>').appendTo('#mock')[0]).config(options);
    });
    testHandlers('MQ.StaticMath::config() propagates down to \\EquatorMathField{}', function(options) {
      return MQ.StaticMath($('<span>\\EquatorMathField{}</span>').appendTo('#mock')[0]).config(options).innerFields[0];
    });
    testHandlers('.config() directly on a \\EquatorMathField{} in a MQ.StaticMath using .innerFields', function(options) {
      return MQ.StaticMath($('<span>\\EquatorMathField{}</span>').appendTo('#mock')[0]).innerFields[0].config(options);
    });
    suite('global MQ.config()', function() {
      ...
      testHandlers('\\EquatorMathField{} in a MQ.StaticMath', function(options) {
        MQ.config(options);
        return MQ.StaticMath($('<span>\\EquatorMathField{}</span>').appendTo('#mock')[0]).innerFields[0];
      });
      teardown(function() {
